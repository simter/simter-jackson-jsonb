...
/**
 * Test JSON-B serialize JavaTime with simter JavaTimeModule.
 *
 * @author RJ
 */
class SimterJavaTimeModuleTest @Autowired constructor() {
  data class DateTimeExample(
    val localDate: LocalDate,
    @get:DateTimeFormat(pattern = "yyyy年MM月dd日") // 自定义格式，使用 JsonFormat 也可以
  ...
  private val now2Minute = now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"))!!
  private val now2Second = now.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))!!
  ...
  fun `single value`() {
    ...
    assertEquals("\"2010-01-02 13:20\"", JsonbBuilder.create().toJson(localDateTime))
  }
  ...
  fun `bean value`() {
    ...
    assertThatJson(json).and(
      ...
      JsonAssertion { it.node("localTime").isEqualTo(now2Second.substring(11, 16)) }, // HH:mm
      JsonAssertion { it.node("localDateTime").isEqualTo(now2Minute) } // yyyy-MM-dd HH:mm
    )
  }
}